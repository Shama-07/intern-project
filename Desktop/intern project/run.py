...
def product_update():
    if request.method == 'POST':
        ...
        product_price = request.form['product_price']
        if product_name.strip() is None and product_price.strip() is None:
            return render_template('product_update.html', error="Product name and price mandatory")
        else:
            try:
                with sql.connect("database.db") as con:
                    cur = con.cursor()
                    ...
                    if rows:
                        query = ''' UPDATE products
                                    SET price = ? 
             		            WHERE name = ?'''
                        cur.execute(query,(product_price,product_name))
                        con.commit()
                        msg = "Record updated successfully"
                    else:
                        msg="Record not found in table"
            except:
                con.rollback()
                msg = "Error in updating record"
            finally:
                con.close()
                return render_template("product_update.html", msg=msg)
    else:
